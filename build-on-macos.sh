...
export openssl_prefix=/opt/homebrew/Cellar/openssl@1.1/1.1.1t/
export zlib_prefix=/opt/homebrew/Cellar/zlib/
...
if ([ $# -gt 0 ] && [ "$1" == "latest" ]) || [ "$version" == "latest" ]; then
    ...
    OR_PREFIX=${OR_PREFIX:="/opt/homebrew/bin/openresty-debug"}
else
    ...
    OR_PREFIX=${OR_PREFIX:="/opt/homebrew/bin/openresty"}
fi
